...
static void setup_gamma_regs(struct s5p_lcd *lcd, u16 gamma_regs[])
{
	...
	for (c = 0; c < 3; c++) {
		...
		scaled_offset = DIV_ROUND_CLOSEST(hacky_v1_offset[c] * adj, 90);
...
static int tl2796_suspend(struct device *dev)
{
	struct spi_device *spi = container_of(dev, struct spi_device, dev);

	struct s5p_lcd *lcd = dev_get_drvdata(&spi->dev);

	tl2796_ldi_disable(lcd);

	return 0;
}
static int tl2796_resume(struct device *dev)
{
	struct spi_device *spi = container_of(dev, struct spi_device, dev);
...
static const struct dev_pm_ops tl2796_pm_ops = {
	SET_SYSTEM_SLEEP_PM_OPS(tl2796_suspend, tl2796_resume)
};
